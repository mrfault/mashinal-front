   ...
   <div class="container">
      <div class="row">
         <template v-for="child in user.children">
            ...
            <div class="col-12 col-lg-4 my-3" @click="loginToSubAccount(child.id, child.part_salon)"
                 v-if="child.autosalon || child.part_salon">
               <salon-login-card v-if="child.autosalon" :count="auto_count(child)" :salon="child.autosalon"
                                 :shop="false"/>
               <salon-login-card v-if="child.part_salon" :count="child.parts_count" :salon="child.part_salon"
                                 :shop="true"/>
            </div>
         </template>
      </div>
