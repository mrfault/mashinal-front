...
export default {
  ...
  computed: {
    ...
    tokens() {
      let tokens = {
        ...
        parts_unlimited: this.loggedIn ? true : this.sellTokens.parts_unlimited,
  ...
  methods: {
    handleVehicleType(e) {
      ...
      e.value === "registration_marks"
        ? (this.vehicleType = "/registration-marks/create")
    ...
    getTextLines(phone) {
      let isSalon =
        (this.loggedIn && this.user.autosalon) || this.user.external_salon;
      ...
      let hasPartsTokens = this.user.external_salon
        ? 0
        : partTokens > 0 || this.tokens.parts_unlimited;
      ...
      if (!hasTransportTokens || !hasPartsTokens) {
        if (
          !(secondLocaleEnding && this.user.external_salon) ||
          !this.user.external_salon
        ) {
          secondLine =
        ...
        thirdLine = this.$t("contact_for_more_info", {
          phone: "*8787",
          email: isSalon || isShop ? "sales@al.ventures" : "office@al.ventures",
        });
      }
