<template>
   <div v-if="single_announce">
      ...
      >
         ...
         <!--      seats_count-->
         <section name="seats_count">
            <title-with-line-and-reject-reason
               :id="'anchor-seats_count'"
               reject-key="seats_count"
               required
               title="seat_count"
               @change="changeReason"
            />
...
export default {
   ...
   computed: {
      ...
      ...mapGetters(['sellOptions', 'sellSalonRights', 'staticPages', 'popularOptions','allSellOptions2']),
      isModerator() {
         return this.user.admin_group && (this.user.admin_group == 2);
      },
      n_of_seats(){
         return [
            {"key":1,"name":"Number of seats: 2"},
            {"key":2,"name":"Number of seats: 4"},
