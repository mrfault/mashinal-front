<template>
   <component :is="isMobileBreakpoint ? 'mobile-screen' : 'div'" @back="$emit('close')" @action="$emit('clean')"
              action-icon="reset" :bar-title="title" height-auto>
      <div class="sell_last-step">
         ...
         <div v-if="showAllOptions" :class="{'disabled-content': type === 'cars' && !form.car_catalog_id}">
            <h2 class="title-with-line mt-3 mt-lg-0" id="anchor-saved_images">
               <span>{{ $t('photos') }} ({{
                     $t('at_least_5_photos', {min: minFiles, max: maxFiles}).toLowerCase()
            ...
            <template v-if="user.autosalon && user.autosalon.access_to_360 && type === 'cars'">
               ...
               <div class="d-flex align-items-center">
                  ...
                  <label for="file" :class="{pending: videoUploading }"
                         class="btn btn--dark-blue-2">{{ $t('video_choose') }}</label>
                  <span class="ml-1" v-if="!videoUploaded && uploadPercentage">{{ uploadPercentage }}%</span>
                  <span v-if="videoUploaded" class="ml-2" style="margin-bottom: 4px;"><icon
                     style="color:green;font-size: 20px;" name="check-circle"/></span>
               </div>
            </template>
            ...
            <div class="row">
               ...
               <div class="col-lg-auto mb-2 mb-lg-0">
                  <div class="d-flex flex-wrap flex-lg-nowrap">
                     ...
                     <form-checkbox transparent :label="$t('bitie')" v-model="form.beaten" input-name="beaten"
                                    has-popover>
                        <popover class="white-space-pre-wrap-span"
                                 :message="$t('with_significant_damage_to_body_elements_that_do_not_move_on_their_own')"
                                 :width="175"/>
                     </form-checkbox>
                     <form-checkbox v-if="!user.external_salon" transparent :label="$t('not_cleared')"
                                    v-model="form.customs_clearance" input-name="customs_clearance"
                                    @change="removeError('car_number', true), removeError('vin', true), form.car_number = ''"/>
                     <form-checkbox v-if="!user.external_salon" transparent :label="$t('in_garanty')"
                                    v-model="form.guaranty" input-name="guaranty"/>
                  </div>
               </div>
            </div>
            ...
            <template v-if="user.external_salon">
               ...
               <div class="row">
                  <div class="col-lg-4 mb-2 mb-lg-0">
                     <form-select :label="$t('country')" :options="sellOptions.countries" v-model="form.country_id"
                                  has-search
                                  :invalid="isInvalid('region_id')"
                                  @change="removeError('region_id'), updatePreview('region')"
                                  :clear-option="false"/>
            ...
            <div class="row">
               ...
               <div v-if="!user.external_salon" class="col-lg-auto mb-2 mb-lg-0">
                  <div class="d-flex flex-wrap flex-lg-nowrap">
                     <form-checkbox transparent :label="$t('tradeable')" v-model="form.tradeable"
                                    input-name="tradeable"/>
                     <form-checkbox transparent :label="$t('credit_possible')" v-model="form.credit"
                                    input-name="credit"/>
                  </div>
               </div>
            </div>
                  ...
                  <div class="col-lg-auto mb-2 mb-lg-0">
                     <div class="row">
                        <div class="col-lg-auto flex-grow-1"
                             :style="isMobileBreakpoint ? '    max-width: 57%; margin-bottom:10px;': ''">
                           <form-switch @change="removeError('end_date')" auto-width v-model="form.auction"
                                        :options="[{ name:$t('auction'), key:1 }, { name:$t('sell'), key:0 }]"
            ...
            <!--          </div>-->
            <!--        </div>-->
            <h2 v-if="type === 'cars' || (type !=='parts' && user.external_salon)" class="title-with-line mt-2 mt-lg-3"
                id="anchor-car_or_vin">
          <span id="anchor-vin">{{
                $t(form.customs_clearance || user.external_salon ? 'vin_carcase_number' : 'license_plate_number_vin_or_carcase_number')
             }}
            <template
               v-if="!loggedIn || (loggedIn && !user.autosalon) || (loggedIn && user.autosalon && user.autosalon.is_official) || (user.external_salon)">
               <span class="star" v-if="type === 'cars' || (type !=='parts' && user.external_salon)"> *</span>
            </template>
          </span>
            </h2>
            <div class="row" id="anchor-car_number"
                 v-if="type === 'cars' && !user.is_autosalon || (type !=='parts' && user.external_salon)">
               <div class="col-lg-4 mb-2 mb-lg-0" v-if="!form.customs_clearance && !user.external_salon">
                  ...
                  <form-checkbox :label="$t('show_car_number_on_site')" v-model="form.show_car_number"
            ...
            <div class="mt-2 mt-lg-3">
               <template v-if="type === 'cars'">
                  <car-filters :values="form.all_options" @change-filter="updateCarFilter" popular
            ...
            <div class="publish-post mb-4" id="anchor-finish">
               ...
               <p class="info-text full-width mt-2">
                  ...
                  <nuxt-link :to="`/page/${getRulesPage.slug[locale]}`" @click.native.prevent="showRules = true"
                             event="">
      ...
      updatePreview(key) {
         if (!key || key === 'region')
            this.setSellPreviewData({
               value: this.sellOptions.regions
            ...
            if (status === 420) {
               ...
               if (dataLength) {
                  ...
                  for (let key in data) {
                     ...
                     // show error
                     this.showError(errorKey, errorText, {
                        fieldView: key,
                        offset: this.isMobileBreakpoint ? 30 : -20
                     }, count === 0);
                     count++;
                  }
