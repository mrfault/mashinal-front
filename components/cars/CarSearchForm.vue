<template>
  <div class="cars-search-form form">
          ...
          <div class="row">
            <div class="col-4" v-if="!onlySavedSearch">
              <nuxt-link custom exact :to="$localePath('/cars')" v-slot="{ href }">
                <a :href="href" class="btn btn--pale-red full-width"
                   :class="{'active': ['cars','index'].includes(routeName)}" @click.prevent="goToSearch(href)">
                  <icon name="search"/>
                  {{ $t('regular_search') }}
                </a>
              </nuxt-link>
            ...
            <div class="col-4" v-if="!onlySavedSearch">
              <nuxt-link custom exact :to="$localePath('/cars/advanced-search')" v-slot="{ href }">
                <a :href="href" class="btn btn--pale-red full-width"
                   :class="{'active': routeName === 'cars-advanced-search'}" @click.prevent="goToSearch(href)">
                  <icon name="options"/>
                  {{ $t('advanced_search') }}
                </a>
              </nuxt-link>
            ...
            <div class="col-4" v-if="!onlySavedSearch">
              <nuxt-link custom exact :to="$localePath('/cars/assistant')" v-slot="{ href }">
                <a :href="href" class="btn btn--pale-red full-width" :class="{'active': routeName === 'cars-assistant'}"
        ...
        <template v-else>
          <template v-if="isMobileBreakpoint">
            <div class="col-6 mb-2">
              <form-select :label="$t('mark')" :options="brands" v-model="form.additional_brands[rows[0]]['brand']"
                           @change="setBrand($event, rows[0])" has-search :clear-option="false"
                           :popular-options="[129,483,8,1,767,117]"
                           img-key="transformed_media"/>
            </div>
            <div class="col-6 mb-2">
              <form-select :label="$t('model')" :options="carModels[rows[0]]"
                           v-model="form.additional_brands[rows[0]]['model']"
                           :disabled="form.additional_brands[rows[0]]['brand'] && !carModels[rows[0]].length"
                           @change="setModel($event, rows[0])" has-search/>
            </div>
            <div class="col-6 mb-2">
              <form-select :label="$t('generation')" :options="carGenerations[rows[0]]"
                           v-model="form.additional_brands[rows[0]]['generation']"
                           :disabled="form.additional_brands[rows[0]]['model'] && !carGenerations[rows[0]].length"
                           @change="setGeneration($event, rows[0])" has-search has-generations/>
            </div>
          </template>
          <template v-else>
          ...
          <template v-if="advanced">
            <div class="col-6 col-lg-2 mb-2 mb-lg-3">
              ...
              >
                <div class="form-merged">
                  <form-numeric-input :placeholder="$t('from')" v-model="form.mileage_from"
                  ...
                  <div class="col-lg-3 mb-lg-0">
                    ...
                    >
                      <div class="form-merged">
                        <form-numeric-input :placeholder="$t('from')" v-model="form.mileage_from"
                                            :suffix="$t('char_kilometre')"/>
                        <form-numeric-input :placeholder="$t('to')" v-model="form.mileage_to"
                                            :suffix="$t('char_kilometre')"/>
                      </div>
                    </form-select>
                  </div>
                ...
                <template v-if="!onlySavedSearch">
                  <div class="col-lg-3 mt-2 mt-lg-0 mb-3" v-show="searchApplied">
                    <form-checkbox :label="$t('search_save')" v-model="savedSearch" skip-truncate
                                   input-name="savedSearch" transparent :disabled="!loggedIn"
    ...
    <div v-if="!onlySavedSearch" class="announcements-sorting"
    ...
    >
      <form class="form" @submit.prevent="updateNotifications" novalidate>
        ...
        <qrcode-box/>
        <button type="submit" :class="['btn btn--green full-width', { pending2 }]">
          {{ $t('confirm') }}
        </button>
      </form>
    </modal-popup>
...
export default {
  ...
  computed: {
    ...mapGetters(['brands', 'carModels', 'carGenerations', 'bodyOptions', 'sellOptions', 'allSellOptions2', 'singleSavedSearch']),
