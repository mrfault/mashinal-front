        ...
        <template v-if="!finished">
          <div class="mt-2 mt-lg-0 mb-2 mb-lg-3">
            <h4 v-if="currentRealStep !== 10">{{ stepInfoTitle }}
              ...
              >
                <img :src="`/img/docs/${img}.jpg`" alt="" v-for="img in (stepHints[currentRealStep].imgs || [])"
                     :key="img"/>
              </popover>
            </h4>
          </div>
    ...
    >
      <div class="attorney-before-step1">
        ...
        <template v-if="Object.keys(vehicles).length">
          ...
          <vue-scroll>
            <div class="attorney_scrollview">
              <h4 class="sub-category" v-if="vehicles.ownVehicles.length">{{ $t('own_vehicles') }}</h4>
              <div class="vehicle-list">
                <div class="row">
                  <div class="col-md-4 col-6 mb-2" v-for="(item,index) in vehicles.ownVehicles" :key="index">
                    <div class="garage_car-item car-active attorney-vehicle" @click="selectVehicle(item)">
                      <div class="car-bg no-img car-bg-custom-size"
                           :style="item.image ? `background-size: 75%;background-image: url('${item.image}')` : ''">
                      </div>
                      <div class="car-info vehicle-info"
                           :class="{active: stepSendData.carNumber === item.vehicleNumber}">
        ...
        <div class="d-flex justify-content-end">
          <button @click="continueToSteps" class="btn btn-primary btn--green" style="padding: 0 50px;"
                  :class="{ pending }">{{ $t('go_further') }}
          </button>
        </div>
      </div>
    </modal-popup>
...
export default {
  ...
  methods: {
    ...
    async getVehicles(isGeneral) {
      ...
      this.pending = true;
      const data = await this.$axios.$get('/attorney/get_vehicle_list/' + isGeneral)
      this.pending = false;
