      ...
      methods: {
            ...
            if (this.checkBounds) {
               this.map.events.add('boundschange', (e) => {
                  let list = this.salonsList.filter(salon => salon.lat).filter(salon => {
                     ...
                     return ymaps.util.bounds.containsPoint(bounds, [salon.lat, salon.lng]) && salon.announcement_count;
                  });
         ...
         updatePlacemarks(filter = true) {
            ...
            this.objectManager.add({
               ...
               features: this.salonsList.filter(salon => salon.lat && salon.announcement_count).map((salon) => ({
                  type: 'Feature',
                  id: salon.id,
                  geometry: {
                     type: 'Point',
