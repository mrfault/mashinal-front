  ...
  <div class="upload-image" :class="{ 'dragover': onDragover }">
    ...
    >
      ...
      <template v-if="isMobileBreakpoint">
        <div class="col-6 col-lg-1-5 mb-2 mb-lg-3" key="loading" v-if="filesLength !== filesLoadedLength">
          <div class="upload-image_thumbnail">
            <loader />
          </div>
        </div>
      ...
      <template v-else>
        <template v-for="(img, index) in helpers.slice(filesLoadedLength)">
          <div class="col-6 col-lg-1-5 mb-2 mb-lg-3" :key="img">
            <div class="upload-image_thumbnail" v-if="(index + 1) <= (filesLength - filesLoadedLength)">
              <loader />
            </div>
            <div class="upload-image_thumbnail helper" @click.stop="input.click()" v-else>
              <img :src="img" alt=""/>
            </div>
          </div>
        </template>
        <template v-if="filesLength > helpers.length">
          <div class="col-6 col-lg-1-5 mb-2 mb-lg-3" :key="'loading_'+i" v-for="i in (filesLength - filesLoadedLength - helpers.slice(filesLoadedLength).length)">
            <div class="upload-image_thumbnail">
              <loader />
            </div>
          </div>
      ...
      <template v-if="canUpload && (isMobileBreakpoint || (filesLength >= helpers.length))">
    ...
    <input type="file" :id="'upload-image_input--' + inputId" accept="image/*" hidden multiple />
  </div>
</template>
  ...
  methods: {
    ...
    fileDrop(e) {
      ...
      for (let i = 0; i < droppedFiles.length; i++) {
        ...
        this.$set(this.files, key, { name: droppedFiles[i].name, file: droppedFiles[i], loaded: false });
