<template>
  <div class="popover-wrapper">
    ...
    >
      <span class="popover-content_triangle pointer-events-none">
        <icon name="triangle" />
      </span>

      <p :class="['popover-message pointer-events-none', textClass || '']">
...
<script>
   export default {
     ...
     methods: {
       ...
       handleClick(e) {
         let hide = true;
         let isPopoverClicked = ["popover-message", "popover-content"].filter(
           (c) =>
             e.target.classList.contains(c) ||
             this.getParentByClassName(e.target, c)
         ).length;

         if (
           isPopoverClicked ||
           e.target.classList.contains(this.name + "-show-popover")
         )
           hide = false;
