...
<!--         </div>-->
         ...
         <div class="col d-flex align-items-center justify-content-end" v-if="type === 1">
            <span
               v-if="false"
            ...
            <span :class="['control-icon cursor-pointer text-hover-red', {'disabled-ui': !selected.length}]"
               ...
               >
                  <form class="form" @submit.prevent="deactivateAnouncement('announcement')" novalidate>
            ...
            <span :class="['control-icon cursor-pointer text-hover-red', {'disabled-ui': !selected.length}]"
               ...
               >
                  <form class="form" @submit.prevent="deleteAnouncement('announcement')" novalidate>
                     <button type="submit" :class="['btn btn--green full-width', { pending }]">
                        {{ $t('confirm') }}
                     </button>
                  </form>
         ...
         <div class="col d-flex align-items-center justify-content-end" v-else>
            <span :class="['control-icon cursor-pointer text-hover-red', {'disabled-ui': !selectedMarks.length}]"
                  @click="showDeactivateModal = true">
               <!-- <icon name="minus-circle" /> -->
               <inline-svg src="/icons/minus-circle.svg" :height="14"/>
...
<script>
   ...
   export default {
      ...
      methods: {
         ...
         selectMark(mark_id_unique) {
            let detectDuplicate;

            this.selectedMarks.forEach((item, i) => {
               if (item === mark_id_unique) {
                  this.selectedMarks.splice(i, 1);
                  detectDuplicate = true;
               }
            })

            if (!detectDuplicate) this.selectedMarks.push(mark_id_unique)
         },
         ...
         async deactivateAnouncement(type) {
            ...
            if (type === 'announcement') {
               ...
               this.selectedMarks.map(id => {
                  let item = this.getMyPlates.data.find(item => item.id_unique == id);
                  if (![0, 1].includes(item.status)) canProceed = false;
               });
            }
            ...
            try {
               if (type === 'announcement') {
                  ...
                  await Promise.all(this.selectedMarks.map(this.deactivateMyAnnounement));
               }
      ...
      watch: {
         clearSelected() {
            this.selected = this.selectedMarks = [];
         }
      },

      async asyncData({ store }) {
         await store.dispatch('fetchPlates');
      },

      mounted() {
         this.$nuxt.$on('select-announcement', this.selectAnnouncement);
         this.$nuxt.$on('select-mark', this.selectMark);
      },

      beforeDestroy() {
         this.$nuxt.$off('select-announcement', this.selectAnnouncement);
         this.$nuxt.$off('select-mark', this.selectMark);
      }
   }
</script>
