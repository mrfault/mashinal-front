   ...
   >
      ...
      >
         ...
         <div class="ads_statics p-5">
            <div>
               <span
                  v-if=" loggedIn &&  announcement.user_id== $auth.user.id && announcement.monetization && announcement.monetization.end_at">{{ $t('announcement_monetization_is_paid', {date: getOnlyDate(announcement.monetization.end_at)}) }} <br></span>
               {{ $t('ads_title') }}
            </div>
         ...
         <div class="radio-items">
            ...
            <label class="radio-container" v-for="(priceItem,index) in priceList">
               ...
               <input type="radio" :checked="index==0 ? 'checked' : null" name="package"
                      @change="selectPackage(priceItem)">
         ...
         <div class="d-flex">
            ...
            <label class="radio-container"
                   v-if="this.$auth.loggedIn && this.user.balance > minimumPrice  && this.user.balance>this.price.value">
               {{ $t('balans') }}
               <input type="radio" name="payment_type" :checked="paymentMethod=='balance'"
...
export default {
   ...
   methods: {
      ...
      selectPackage(priceItem) {

         this.selectedPlan = priceItem
         this.day.value = priceItem.day
         this.price.value = priceItem.price

         if (this.user.balance < priceItem.price && this.paymentMethod == 'balance') {
   ...
   beforeCreate() {
      this.$axios.$get('/monetization/price/list').then((res) => {
         ...
         this.price.value = this.priceList[0].price
         this.day.value = this.priceList[0].days
         this.selectedPlan = this.priceList[0]
